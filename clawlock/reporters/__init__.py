"""ClawLock v2.4.0 report renderer - Rich terminal + JSON + HTML output."""
...
_LEVEL_ORDER = {CRIT: 0, HIGH: 1, WARN: 2, INFO: 3}
...
def _domain_score(findings: List[Finding], *, domain: str = "") -> int:
    ...
    Then exponential decay on WARN count and a per-extra-HIGH deduction so
    multiple HIGHs in the same domain do not collapse onto the same number.
    ...
    nh = sum(1 for f in findings if f.level == HIGH)
    nw = sum(1 for f in findings if f.level == WARN)

    cap = 50 if nh else 99
    raw = 100.0 * math.exp(-_WARN_DECAY * nw)
    raw = min(cap, raw)
    raw -= max(0, nh - 1) * _HIGH_DEDUCTION
...
def _build_domain_report(
    ...
    *,
    ...
    has_real_findings = any(f.level in (CRIT, HIGH, WARN) for f in accountable)
...
def render_scan_report(
    ...
    findings_summary = []
    for f in all_f:
        if f.level not in (CRIT, HIGH, WARN):
            continue
        entry = {"title": f.title, "level": f.level, "location": f.location}
        ids = (f.metadata or {}).get("measure_ids") if isinstance(f.metadata, dict) else None
        if ids:
...
def _render_html(
    ...
    _footer1 = t("由 ClawLock v2.4.0 生成", "Generated by ClawLock v2.4.0")

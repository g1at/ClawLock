...
- **能力链关联**：组合“敏感读取 → 对外写入”“下载 → 执行”“记忆写入 → 自启动”“不可信路径 → 写入/执行”等事件。结构化数据流证据保留原有等级和置信度；仅由文本推测的关联标记为未确认、中危待复核。可解析的 Python 文本按词法作用域隔离，并排除注释和文档字符串中的关联证据。
...
指定 `--verify` 后，即使没有应用修改，也会检查配置和凭证。验证完整且无严重/高危发现时退出 `0`，仍有严重/高危时退出 `1`，任一检查失败或不完整时退出 `2`。不完整状态优先，不能显示为验证通过。
...
配置回滚会逐文件核对本次操作写入的确切内容。后续修改会保留，日志和备份留供核对。旧配置日志如果缺少写后内容摘要，也需要人工核对；权限回滚继续支持。
